...
threshold={self.threshold}]\n{left}{rigth}")
    ...
    def get_leaves_below(self):
        """
            Returns a list of all the leaves of a tree
        """
    ...
    def update_indicator(self):
        ...
        def is_large_enough(x):
            ...
            return np.all(np.array([np.greater(x[:, key], self.lower[key])
                                    for
                                    key in list(self.lower.keys())]), axis=0)
        ...
        def is_small_enough(x):
            ...
            return np.all(np.array([np.less_equal(x[:, key], self.upper[key])
                                    for
                                    key in list(self.upper.keys())]), axis=0)
    ...
    def pred(self, x):
        """
            Returns either the lef child prediction or the rigth one
        """
...
class Decision_Tree():
    ...
    def update_bounds(self):
        """
            Return the bounds of each node of a tree
        """
